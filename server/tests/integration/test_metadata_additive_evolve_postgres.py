...
SCHEMA_V10 = SchemaId.parse(f"{SCHEMA_ID}@1.0.0")
...
class TestAdditiveEvolvePipeline:
    async def test_old_row_null_new_row_typed(
        ...
        await service.ensure_table(SCHEMA_V10, _fields_v10())
        r_old = RecordSRN.parse("urn:osa:localhost:rec:old@1")
        await seed_record(
            pg_engine,
            srn=str(r_old),
            schema_id=SCHEMA_V10.id.root,
            schema_version=SCHEMA_V10.version.root,
        )
        ...
        await service.ensure_table(SCHEMA_V11, _fields_v11())
        r_new = RecordSRN.parse("urn:osa:localhost:rec:new@1")
        await seed_record(
            pg_engine,
            srn=str(r_new),
            schema_id=SCHEMA_V11.id.root,
            schema_version=SCHEMA_V11.version.root,
        )
