"""GetStats query handler — counts come from the statistics store (#219 ph6).

The records total is the SUM over lockstep table_statistics, never a live
full-table COUNT; the snapshot covers storage/feature aggregates.
"""
...
from osa.domain.data.model.statistics import InstanceStats
...
class TestGetStatsHandler:
    ...
    async def test_returns_record_count_from_service(self):
        ...
        stats_store.records_total.return_value = 42
        ...
        handler = GetStatsHandler(stats_store=stats_store)
        ...
        stats_store.records_total.assert_awaited_once()

    @pytest.mark.asyncio
    async def test_falls_back_to_live_compute_before_first_refresh(self):
        stats_store = AsyncMock()
        stats_store.records_total.return_value = 10
