...
_sql = PostgresTableReadStore.statement_timeout_sql


def test_statement_timeout_sql_renders_integer_milliseconds() -> None:
    assert _sql(timedelta(seconds=30)) == "SET LOCAL statement_timeout = '30000ms'"
    assert _sql(timedelta(minutes=30)) == "SET LOCAL statement_timeout = '1800000ms'"


def test_statement_timeout_sql_truncates_subsecond() -> None:
    # int milliseconds — no operator-supplied string ever reaches raw SQL.
    assert _sql(timedelta(milliseconds=1500.7)) == "SET LOCAL statement_timeout = '1500ms'"
