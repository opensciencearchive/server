...
class TestReadHookFeatures:
    ...
    async def test_reads_features_list(self, tmp_path: Path):
        ...
        # The hook_output_dir is the deposition root directory
        dep_root = tmp_path / "depositions" / "localhost_test-dep"
        output_dir = dep_root / "hooks" / "detect" / "output"
        output_dir.mkdir(parents=True)
        (output_dir / "features.json").write_text(json.dumps([{"score": 0.95}, {"score": 0.82}]))

        features = await adapter.read_hook_features(str(dep_root), "detect")
    ...
    async def test_returns_empty_when_missing(self, tmp_path: Path):
        ...
        dep_root = tmp_path / "depositions" / "localhost_test-dep"
        features = await adapter.read_hook_features(str(dep_root), "nonexistent")
...
class TestHookFeaturesExist:
    ...
    async def test_true_when_file_exists(self, tmp_path: Path):
        ...
        assert await adapter.hook_features_exist(str(dep_root), "detect") is True
