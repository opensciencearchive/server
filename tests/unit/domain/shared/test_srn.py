import pytest
from osa.domain.shared.model.srn import SRN, RecordSRN, DepositionSRN, SchemaSRN, ResourceType

class TestSRN:
    def test_parse_record_srn(self):
        raw = "urn:osa:node-1:rec:123@1"
        srn = SRN.parse(raw)
        assert isinstance(srn, RecordSRN)
        assert srn.type == ResourceType.rec
        assert srn.local.root == "123"
    ...
    def test_parse_schema_srn(self):
        ...
        srn = SRN.parse(raw)
        assert isinstance(srn, SchemaSRN)
        assert srn.type == ResourceType.schema
        assert srn.local.root == "my-schema"
