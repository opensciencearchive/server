...
class SRN(BaseModel):
    ...
    _tpl = Template("urn:osa:${node}:${type}:${local}${ver}")
    ...
    def render(self) -> str:
        ...
        if self.version is not None:
            # RootModel stores the primitive in .root
            v = self.version.root if isinstance(self.version, RootModel) else self.version
            ver = f"@{v}"
        return self._tpl.substitute(node=self.node.root, type=self.type.value, local=self.local.root, ver=ver)

    # ---------- factory & parsing ----------

    @classmethod
    def parse(cls, srn: str) -> SRN:
        """
        ...
        parts = srn.split(":")
        if len(parts) < 5:
            raise ValueError("malformed SRN (expected urn:osa:{node}:{type}:{local}[...])")
        _, _, node, typ, rest = parts[0], parts[1], parts[2], parts[3], ":".join(parts[4:])
       ...
       # dispatch to stricter subclasses
        ...
        sub_cls: Type[SRN] = {
            ...
            ResourceType.assur: AssuranceSRN,
        ...
        return sub_cls(node=NodeId(node), type=ResourceType(typ), local=LocalId(local_str), version=version)

# ---------- Per-type SRNs (tighten version constraints) ----------

class RecordSRN(SRN):
    type: ResourceType = Field(default=ResourceType.rec, frozen=True)
    version: Optional[RecordVersion]  # records: integer version or None (if you choose to allow implicit v1)
...
class SchemaSRN(SRN):
    ...
    version: Semver  # required and must be semver


class AssuranceSRN(SRN):
    type: ResourceType = Field(default=ResourceType.assur, frozen=True)
    version: Semver  # required and must be semver
